...
/* ── Map markers that react to analysis mode ── */
    ...
    <>
      {districts.map((d) => {
        ...
        if (mode === "region") {
          ...
          // green (good) to red (bad accessibility)
...
/* ── Region summary overlays on map ── */
function RegionSummaryOverlay({ districts, categoryLabels }) {
  ...
  return (
    <div className="absolute bottom-4 left-4 z-[1000] flex flex-wrap gap-2 max-w-[600px]">
      {regionData.map((r) => (
        <div
          key={r.name}
          className="bg-white/40 backdrop-blur-xl border border-white/40 rounded-xl shadow-lg px-3 py-2 min-w-[140px]"
        >
          ...
          <p className="text-[11px] text-gray-600 font-semibold mb-1">총 {r.total}개 시설</p>
...
function HeatmapLegend({ category, districts }) {
  ...
  return (
    <div className="absolute bottom-4 left-4 z-[1000] bg-white/40 backdrop-blur-xl border border-white/40 rounded-xl shadow-lg px-4 py-3">
...
/* ── District detail tooltip on map click ── */
function DistrictDetail({ district, categoryLabels, onClose }) {
  ...
  return (
    <div className="absolute top-20 right-4 z-[1000] bg-white/40 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl p-4 w-[260px]">
...
export default function AnalyticsPage() {
  ...
  const [mode, setMode] = useState("cluster"); // cluster | region | heatmap | accessibility
  ...
  /* Accessibility: count subway stations within 0.5km of each district center */
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      ...
      {mode === "accessibility" && (
        <div className="absolute bottom-4 left-4 z-[1000] bg-white/40 backdrop-blur-xl border border-white/40 rounded-xl shadow-lg px-4 py-3 max-w-[500px]">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="h-3 w-32 rounded-full" style={{ background: "linear-gradient(to right, #ef4444, #f59e0b, #22c55e)" }} />
      ...
      <div className="relative z-10 pointer-events-none w-full h-full flex flex-col p-4 gap-4">
        ...
        <div className="flex-1 flex gap-4 min-h-0">
          {/* Left sidebar — mode controls + analysis panels */}
          <aside className="pointer-events-auto w-[320px] flex-shrink-0 flex flex-col gap-3 overflow-y-auto">
            {/* Title */}
